...
const INTEGRATION_NAME = "google_search_console";

const INTEGRATION_TOOLS: any[] = [
{
    name: "google_search_console_request",
    description:
      "Execute a Google Search Console API request via Google SDK. Google Search Console (sites, search analytics, sitemaps, URL inspection). " +
      "Provide resource path, method name, and params/body.",
    parameters: Type.Object({
      agent_id: Type.String({ description: "Agent with Google Search Console integration" }),
    ...
    async execute(_id: string, p: any) {
      return json(await post("/integrations/search-console/request", p));
    },
  }
];
