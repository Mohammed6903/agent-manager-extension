...
export function register(api: any) {
  api.registerTool({
    name: "mailchimp_request",
    description:
      "Execute an API request against Mailchimp. Mailchimp email marketing (audiences, members, campaigns). " +
      "Provide the HTTP method, path, and optional params/body.",
    parameters: Type.Object({
      agent_id: Type.String({ description: "Agent with Mailchimp integration" }),
    ...
    async execute(_id: string, p: any) {
      return json(await post("/integrations/mailchimp/request", p));
    },
  });
}
