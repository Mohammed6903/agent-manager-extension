...
const INTEGRATION_NAME = "monday";

const INTEGRATION_TOOLS: any[] = [
{
    name: "monday_request",
    description:
      "Execute an API request against Monday. Monday.com work management (GraphQL API — send query in json_body). " +
      "Provide the HTTP method, path, and optional params/body.",
    parameters: Type.Object({
      agent_id: Type.String({ description: "Agent with Monday integration" }),
    ...
    async execute(_id: string, p: any) {
      return json(await post("/integrations/monday/request", p));
    },
  }
];
