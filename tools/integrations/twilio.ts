...
export function register(api: any) {
  api.registerTool({
    name: "twilio_request",
    description:
      "Execute an API request against Twilio. Twilio communications (SMS messages, phone calls, phone numbers). Uses form-encoded POST bodies.. " +
      "Provide the HTTP method, path, and optional params/body.",
    parameters: Type.Object({
      agent_id: Type.String({ description: "Agent with Twilio integration" }),
    ...
    async execute(_id: string, p: any) {
      return json(await post("/integrations/twilio/request", p));
    },
  });
}
