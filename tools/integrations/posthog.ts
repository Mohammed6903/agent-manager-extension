...
const INTEGRATION_NAME = "posthog";

const INTEGRATION_TOOLS: any[] = [
{
    name: "posthog_request",
    description:
      "Execute an API request against PostHog. PostHog product analytics (events, persons, feature flags, insights, dashboards, HogQL queries). " +
      "Provide the HTTP method, path, and optional params/body.",
    parameters: Type.Object({
      agent_id: Type.String({ description: "Agent with PostHog integration" }),
    ...
    async execute(_id: string, p: any) {
      return json(await post("/integrations/posthog/request", p));
    },
  }
];
