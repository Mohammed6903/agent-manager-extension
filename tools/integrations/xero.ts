...
const INTEGRATION_NAME = "xero";

const INTEGRATION_TOOLS: any[] = [
{
    name: "xero_request",
    description:
      "Execute an API request against Xero. Xero accounting (contacts, invoices, accounts, bank transactions, payments). " +
      "Provide the HTTP method, path, and optional params/body.",
    parameters: Type.Object({
      agent_id: Type.String({ description: "Agent with Xero integration" }),
    ...
    async execute(_id: string, p: any) {
      return json(await post("/integrations/xero/request", p));
    },
  }
];
