...
const INTEGRATION_NAME = "square";

const INTEGRATION_TOOLS: any[] = [
{
    name: "square_request",
    description:
      "Execute an API request against Square. Square payments/POS (locations, payments, customers, orders, invoices, catalog). " +
      "Provide the HTTP method, path, and optional params/body.",
    parameters: Type.Object({
      agent_id: Type.String({ description: "Agent with Square integration" }),
    ...
    async execute(_id: string, p: any) {
      return json(await post("/integrations/square/request", p));
    },
  }
];
