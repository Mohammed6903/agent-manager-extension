...
const INTEGRATION_NAME = "chargebee";

const INTEGRATION_TOOLS: any[] = [
{
    name: "chargebee_request",
    description:
      "Execute an API request against Chargebee. Chargebee subscription billing (customers, subscriptions, invoices, plans). " +
      "Provide the HTTP method, path, and optional params/body.",
    parameters: Type.Object({
      agent_id: Type.String({ description: "Agent with Chargebee integration" }),
    ...
    async execute(_id: string, p: any) {
      return json(await post("/integrations/chargebee/request", p));
    },
  }
];
