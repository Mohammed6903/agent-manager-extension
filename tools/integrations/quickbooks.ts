...
const INTEGRATION_NAME = "quickbooks";

const INTEGRATION_TOOLS: any[] = [
{
    name: "quickbooks_request",
    description:
      "Execute an API request against QuickBooks. QuickBooks accounting (customers, invoices, payments, bills, vendors, SOQL-like queries). " +
      "Provide the HTTP method, path, and optional params/body.",
    parameters: Type.Object({
      agent_id: Type.String({ description: "Agent with QuickBooks integration" }),
    ...
    async execute(_id: string, p: any) {
      return json(await post("/integrations/quickbooks/request", p));
    },
  }
];
