...
const INTEGRATION_NAME = "whatsapp";

const INTEGRATION_TOOLS: any[] = [
{
    name: "whatsapp_request",
    description:
      "Execute an API request against WhatsApp Business. WhatsApp Business via Meta Graph API (send messages, manage templates, media). " +
      "Provide the HTTP method, path, and optional params/body.",
    parameters: Type.Object({
      agent_id: Type.String({ description: "Agent with WhatsApp Business integration" }),
    ...
    async execute(_id: string, p: any) {
      return json(await post("/integrations/whatsapp/request", p));
    },
  }
];
