...
const INTEGRATION_NAME = "sentry";

const INTEGRATION_TOOLS: any[] = [
{
    name: "sentry_request",
    description:
      "Execute an API request against Sentry. Sentry error monitoring (organizations, projects, issues, events). " +
      "Provide the HTTP method, path, and optional params/body.",
    parameters: Type.Object({
      agent_id: Type.String({ description: "Agent with Sentry integration" }),
    ...
    async execute(_id: string, p: any) {
      return json(await post("/integrations/sentry/request", p));
    },
  }
];
