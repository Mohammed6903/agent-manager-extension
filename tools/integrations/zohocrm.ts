...
const INTEGRATION_NAME = "zohocrm";

const INTEGRATION_TOOLS: any[] = [
{
    name: "zohocrm_request",
    description:
      "Execute an API request against Zoho CRM. Zoho CRM (modules: Leads, Contacts, Deals — CRUD and search). " +
      "Provide the HTTP method, path, and optional params/body.",
    parameters: Type.Object({
      agent_id: Type.String({ description: "Agent with Zoho CRM integration" }),
    ...
    async execute(_id: string, p: any) {
      return json(await post("/integrations/zohocrm/request", p));
    },
  }
];
