...
const INTEGRATION_NAME = "sendgrid";

const INTEGRATION_TOOLS: any[] = [
{
    name: "sendgrid_request",
    description:
      "Execute an API request against SendGrid. SendGrid email delivery (send emails, templates, contacts, stats). " +
      "Provide the HTTP method, path, and optional params/body.",
    parameters: Type.Object({
      agent_id: Type.String({ description: "Agent with SendGrid integration" }),
    ...
    async execute(_id: string, p: any) {
      return json(await post("/integrations/sendgrid/request", p));
    },
  }
];
