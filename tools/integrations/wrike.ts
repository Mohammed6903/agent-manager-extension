...
const INTEGRATION_NAME = "wrike";

const INTEGRATION_TOOLS: any[] = [
{
    name: "wrike_request",
    description:
      "Execute an API request against Wrike. Wrike project management (folders, tasks, spaces, workflows). " +
      "Provide the HTTP method, path, and optional params/body.",
    parameters: Type.Object({
      agent_id: Type.String({ description: "Agent with Wrike integration" }),
    ...
    async execute(_id: string, p: any) {
      return json(await post("/integrations/wrike/request", p));
    },
  }
];
