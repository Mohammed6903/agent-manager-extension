...
const INTEGRATION_NAME = "google_analytics";

const INTEGRATION_TOOLS: any[] = [
{
    name: "google_analytics_request",
    description:
      "Execute a Google Analytics API request via Google SDK. Google Analytics GA4 (runReport, runRealtimeReport, metadata). " +
      "Provide resource path, method name, and params/body.",
    parameters: Type.Object({
      agent_id: Type.String({ description: "Agent with Google Analytics integration" }),
    ...
    async execute(_id: string, p: any) {
      return json(await post("/integrations/analytics/request", p));
    },
  }
];
