...
const INTEGRATION_TOOLS: any[] = [
{
    name: "jira_request",
    description:
      "Execute an API request against Jira. Jira project management (issues, projects, search with JQL). " +
      "Provide the HTTP method, path, and optional params/body.",
    parameters: Type.Object({
      agent_id: Type.String({ description: "Agent with Jira integration" }),
    ...
    async execute(_id: string, p: any) {
      return json(await post("/integrations/jira/request", p));
    },
  }
];
